...
void ModeGuided::wp_control_run()
{
    ...
    if (auto_yaw.mode() == AUTO_YAW_HOLD) {
        ...
        attitude_control->input_thrust_vector_rate_heading(wp_nav->get_thrust_vector(), auto_yaw.rate_cds(), g2.pilot_rate_y_tc);
    } else {
...
void ModeGuided::pause_control_run()
{
    ...
    // call attitude controller
    attitude_control->input_thrust_vector_rate_heading(pos_control->get_thrust_vector(), 0.0, g2.pilot_rate_y_tc);
}
...
void ModeGuided::angle_control_run()
{
    ...
    if (guided_angle_state.attitude_quat.is_zero()) {
        attitude_control->input_rate_bf_roll_pitch_yaw(ToDeg(guided_angle_state.ang_vel.x) * 100.0f, ToDeg(guided_angle_state.ang_vel.y) * 100.0f, ToDeg(guided_angle_state.ang_vel.z) * 100.0f, g2.acro_rate_rp_tc, g2.acro_rate_y_tc);
    } else {
