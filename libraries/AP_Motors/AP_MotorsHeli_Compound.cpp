...
const AP_Param::GroupInfo AP_MotorsHeli_Compound::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("SWASH_TYPE", 5, AP_MotorsHeli_Compound, _swash_type, AP_MOTORS_HELI_COMPOUND_SWASH_H3),
...
void AP_MotorsHeli_Compound::output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
{
    if (_swash_type == AP_MOTORS_HELI_COMPOUND_SWASH_H3) {                  //Three-Servo adjustable CCPM mixer factors
        // aileron factors
    ...
    } else if (_swash_type == AP_MOTORS_HELI_COMPOUND_SWASH_H3_140) {       //Three-Servo H3-140 CCPM mixer factors
        // aileron factors
        _rollFactor[CH_1] = 1;
        _rollFactor[CH_2] = -1;
        _rollFactor[CH_3] = 0;
...
void AP_MotorsHeli_Compound::move_actuators(float roll_out, float pitch_out, float coll_in, float yaw_out)
{
    ...
    if (_collective_direction == AP_MOTORS_HELI_COMPOUND_COLLECTIVE_DIRECTION_REVERSED){
