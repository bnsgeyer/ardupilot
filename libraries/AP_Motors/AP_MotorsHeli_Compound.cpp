...
const AP_Param::GroupInfo AP_MotorsHeli_Compound::var_info[] = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("TAIL_SPEED", 10, AP_MotorsHeli_Compound, _direct_drive_tailspeed, AP_MOTORS_HELI_COMPOUND_DDVP_SPEED_DEFAULT),
    ...
    // @User: Advanced
    // @Increment: 1
    AP_SUBGROUPINFO(_swashplate, "SW_", 20, AP_MotorsHeli_Compound, AP_MotorsHeli_Swash),
...
bool AP_MotorsHeli_Compound::init_outputs()
{
    if (!initialised_ok()) {
        ...
        for (uint8_t i=0; i<AP_MOTORS_HELI_COMPOUND_NUM_SWASHPLATE_SERVOS; i++) {
        ...
        if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH || _tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPIT_EXT_GOV) {
            _tail_rotor.init_servo();
        } else if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
            // external gyro output
            add_motor_num(AP_MOTORS_HELI_COMPOUND_EXTGYRO);
        }
    }
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPIT_EXT_GOV) {
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
        ...
        SRV_Channels::set_output_min_max(SRV_Channels::get_motor_function(AP_MOTORS_HELI_COMPOUND_EXTGYRO), 1000, 2000);
    }
    ...
    if (_swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_90 || _swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_45) {
        reset_swash_servo(SRV_Channels::get_motor_function(5));
    }
...
void AP_MotorsHeli_Compound::_output_test_seq(uint8_t motor_seq, int16_t pwm)
{
        ...
        case 4:
            ...
            if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
                if (_acro_tail && _ext_gyro_gain_acro > 0) {
                    rc_write(AP_MOTORS_HELI_COMPOUND_EXTGYRO, _ext_gyro_gain_acro);
                } else {
                    rc_write(AP_MOTORS_HELI_COMPOUND_EXTGYRO, _ext_gyro_gain_std);
                }
            }
...
uint32_t AP_MotorsHeli_Compound::get_motor_mask()
{
    // heli uses channels 1,2,3,4 and 8
    // setup fast channels
    uint32_t mask = 1U << 0 | 1U << 1 | 1U << 2 | 1U << 3 | 1U << 4 | 1U << AP_MOTORS_HELI_RSC;
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH || _tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPIT_EXT_GOV) {
        mask |= 1U << AP_MOTORS_HELI_COMPOUND_TAILRSC;
    }
...
void AP_MotorsHeli_Compound::move_actuators(float roll_out, float pitch_out, float coll_in, float yaw_out)
{
    ...
    if (_servo_mode == SERVO_CONTROL_MODE_AUTOMATED && !_heliflags.in_autorotation) {
        ...
        if ((_main_rotor.get_control_output() > _main_rotor.get_idle_output()) && _tail_type != AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
            ...
            _collective_yaw_effect.set(constrain_float(_collective_yaw_effect, -AP_MOTORS_HELI_COMPOUND_COLYAW_RANGE, AP_MOTORS_HELI_COMPOUND_COLYAW_RANGE));
    ...
    if (_swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_90 || _swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_45) {
        _servo6_out = _swashplate.get_servo_out(CH_4,pitch_out,roll_out,collective_out_scaled);
    }
...
void AP_MotorsHeli_Compound::output_to_motors()
{
    ...
    if (_swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_90 || _swashplate.get_swash_type() == SWASHPLATE_TYPE_H4_45) {
        rc_write_swash(AP_MOTORS_MOT_6, _servo6_out);
    }
    if (_tail_type != AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH_CW && _tail_type != AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH_CCW){
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
        ...
        if (_acro_tail && _ext_gyro_gain_acro > 0) {
            rc_write(AP_MOTORS_HELI_COMPOUND_EXTGYRO, 1000 + _ext_gyro_gain_acro);
        } else {
            rc_write(AP_MOTORS_HELI_COMPOUND_EXTGYRO, 1000 + _ext_gyro_gain_std);
        }
    }

    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH_CCW) {
        _servo4_out = -_servo4_out;
        _servo5_out = -_servo5_out;
    }
