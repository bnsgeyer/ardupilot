...
public:
    AP_L1_Control_Heli(AP_AHRS &ahrs, const AP_SpdHgtControl_Heli *spdHgtControl)
        : _ahrs(ahrs)
    ...
    void update_waypoint(const Vector2f &prev_WP, const Vector2f &next_WP, float dist_min = 0.0f);
    void update_loiter(const Vector2f &center_WP, float radius, int8_t loiter_direction);
    ...
    int32_t get_angle_total() const { return loiter.sum_cd; }
...
private:
    ...
    const AP_SpdHgtControl_Heli *_spdHgtControl;
