...
void AC_PosControl_Heli::input_d_accel(const float target_z)
{
...
void AC_PosControl_Heli::input_ned_accel_rate_heading(const Vector3f& thrust_vector, Orientation heading, const float target_z)
{
    ...
    if (strcmp(_motors_heli._get_frame(), "HELI_COMPOUND") == 0) {
        ...
        // rotate accelerations into body forward-right frame
        float accel_forward = thrust_vector.x * _ahrs.cos_yaw() + thrust_vector.y * _ahrs.sin_yaw();
        float accel_right = -thrust_vector.x * _ahrs.sin_yaw() + thrust_vector.y * _ahrs.cos_yaw();
